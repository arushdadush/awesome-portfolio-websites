AOS.init();
/* Project Cards */

const projectcards = document.querySelector(".projectcards");

// Array of object for projects
const projects = [
  {
    title: "Cybersecurity Research (DIS)",
    cardImage: "assets/images/project-page/cybersec.webp",
    description: "Reviewed current research on XAI and adversarial attacks and proposed future research directions.",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "",
    Githublink: "",
  },
  {
    title: "Bike Store Data Analysis",
    cardImage: "assets/images/project-page/bikestore.png",
    description: "Full scale data analysis of bike store sales data, including ETL processes and data cleansing.",
    tagimg: "https://image.flaticon.com/icons/png/512/643/643350.png",
    Previewlink: "",
    Githublink: "",
  },
  {
    title: "Spear Phishing Presentation",
    cardImage: "assets/images/project-page/spearphish.jpg",
    description: "Collaborated with a research team to conduct a deep dive into spear phishing, summarizing findings in a presentation for executive shareholders.",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "",
    Githublink: "",
  },
  {
    title: "Coffee Chain Market Analysis",
    cardImage: "assets/images/project-page/coffeesales.png",
    description: "Conducted an in-depth analysis of sales trends and customer demographics for a coffee chain, using IBM Cognos to create visual insights that optimize marketing and inventory strategies.",
    tagimg: "https://miro.medium.com/max/2800/0*U2DmhXYumRyXH6X1.png",
    Previewlink: "",
    Githublink: "",
  },
  {
    title: "Retail Transaction Modeling/Prediction",
    cardImage: "assets/images/project-page/semmalogo.png",
    description: "Utilized SEMMA framework to build a predictive model for retail store transaction data",
    tagimg:
      "data:image/png;base64,",
    Previewlink: "",
    Githublink: "",
  },
];

// function for rendering project cards data
const showCards = () => {
  let output = "";
  projects.forEach(({ title, cardImage, description }) => {
    output += `       
        <div class="column skill-card card" style="margin: 15px"data-aos="zoom-in-up" data-aos-easing="linear" data-aos-delay="300" data-aos-duration="600" >
          <div class="wrapper" style="background: url(${cardImage}) center / cover no-repeat;">
            <div class="header">
            </div>
            <div class="data">
              <div class="content">
                <div class="title-div">
                  <h1 class="title"><a href="#">${title}</a></h1>
                </div>
                <div class="hover-description" style="display: none;">
                  <p style="color: white; padding: 10px; text-align: center;">
                    ${description}
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>`;
  });
  // Rest of the code...
  projectcards.innerHTML = output;

  // Add hover event listeners
const cards = document.querySelectorAll('.skill-card');
cards.forEach(card => {
  const description = card.querySelector('.hover-description');
  const titleDiv = card.querySelector('.title-div');
  
  card.addEventListener('mouseenter', () => {
    titleDiv.style.display = 'none';
    description.style.display = 'block';
  });
  
  card.addEventListener('mouseleave', () => {
    titleDiv.style.display = 'block';
    description.style.display = 'none';
  });
});
};
document.addEventListener("DOMContentLoaded", showCards);

function myFunction() {
  // Declare variables
  var input, button, i, skillcard, card, title;
  input = document.getElementById("myInput").value;
  input = input.toUpperCase();
  skillcard = document.getElementsByClassName("skill-card");
  card = document.getElementsByClassName("card");
  title = document.getElementsByClassName("title");

  // Loop through all list items, and hide those who don't match the search query
  for (i = 0; i < button.length; i++) {
    if (
      button[i].innerHTML.toUpperCase().includes(input) ||
      title[i].innerHTML.toUpperCase().includes(input)
    ) {
      skillcard[i].style.display = "";
      card[i].style.display = "";
    } else {
      skillcard[i].style.display = "none";
      card[i].style.display = "none";
    }
  }
}
